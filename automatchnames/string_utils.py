...
def tidy_names_in_column(df:pd.DataFrame,col:str):
    df[col] = df[col].swifter.progress_bar(False).apply(_capitalize_first_letter_of_taxon, check_string_is_uppercase=True)
    df[col] = df[col].swifter.progress_bar(False).apply(remove_whitespace_at_beginning_and_end)
